...
def test_old_games_key_is_ignored_without_error(tmp_path):
    # UKE-29 v2 dropped game tracking - a file written by an older version
    # (with a "games" key) must still load cleanly, just without carrying
    # anything from that key forward.
    path = tmp_path / "watch_state.json"
    path.write_text(
        json.dumps({"version": 1, "shows": {}, "games": {"5:game.sfc": {"played": True, "play_count": 3}}}),
        encoding="utf-8",
    )
    ws = WatchState(path)  # must not raise
    root = tmp_path / "a"
...
def test_saved_file_shape(tmp_path):
    ...
    assert "games" not in raw  # no longer written (UKE-29 v2)


def test_all_episodes_returns_a_snapshot(tmp_path):
    ws = WatchState(tmp_path / "watch_state.json")
    root = tmp_path / "dragon"
    ws.record_episode_position(2, root, root / "ep1.mp4", position=10.0, duration=100.0)
...
def _lineup(tmp_path, *, dragon_eps=3, arthur_eps=3):
...
def test_continue_watching_respects_limit(tmp_path):
    lineup = _lineup(tmp_path, dragon_eps=3, arthur_eps=3)
    ws = WatchState(tmp_path / "watch_state.json")
    dragon = next(c for c in lineup if c.number == 2)
    arthur = next(c for c in lineup if c.number == 3)
...
# -- insights_summary() (UKE-29, show-only since UKE-29 v2) ------------------


def test_insights_summary_with_no_watch_state_is_empty(tmp_path):
    lineup = _lineup(tmp_path)
    summary = insights_summary(list(lineup), None)
...
def test_insights_summary_totals(tmp_path):
    lineup = _lineup(tmp_path, dragon_eps=2, arthur_eps=2)
    ws = WatchState(tmp_path / "watch_state.json")
    dragon = next(c for c in lineup if c.number == 2)
    arthur = next(c for c in lineup if c.number == 3)
    ws.mark_episode_watched(2, dragon.config.path, dragon.episodes[0], duration=600.0)
...
def test_insights_summary_untouched_channels_excluded_from_favorite(tmp_path):
    ...
    # default (their watched_minutes would be 0 too, but last_played == 0 is
    # the actual exclusion signal).
    summary = insights_summary(list(lineup), ws)
    assert summary.favorite is None
